...
def decode_group_data(level_json):
    ...
    for group in group_map:
        ...
        group_bytes = group['value']['Struct']['Struct']['RawData']['Array']['value']['Base']['Byte']['Byte']
        if isinstance(group_bytes, NoIndent):
            group_bytes = group_bytes.value
        # Parse group_data as an FArchive
        ...
        if group_type == 'EPalGroupType::Guild':
            guild = {
                'admin_player_uid': reader.read_uuid(),
                'players': []
            }
...
def encode_group_data(level_json):
    ...
    for group in group_map:
        ...
        print(f'Encoding group ID:{p["group_id"]} Type:{p["group_type"]} Name:{p["group_name"]}')
        writer = FArchiveWriter()
        writer.write_uuid_str(p['group_id'])
        writer.write_fstring(p['group_name'])
        writer.write_tarray(instance_id_writer,
        ...
        if p['group_type'] == 'EPalGroupType::Guild':
            ...
            for i in range(len(p['players'])):
                ...
                writer.write_int64(
                    p['players'][i]['player_info']['last_online_real_time'])
                writer.write_fstring(
                    p['players'][i]['player_info']['player_name'])
...
def decode_character_data(level_json):
    char_map = level_json['root']['properties']['worldSaveData']['Struct'][
        'value']['Struct']['CharacterSaveParameterMap']['Map']['value']
    for char in char_map:
        char_bytes = char['value']['Struct']['Struct']['RawData']['Array']['value']['Base']['Byte']['Byte']
...
def encode_character_data(level_json):
    ...
    for char in char_map:
        ...
        char_uid = char['key']['Struct']['Struct']['PlayerUId']['Struct']['value']['Guid']
        char_instance_id = char['key']['Struct']['Struct']['InstanceId']['Struct']['value']['Guid']
